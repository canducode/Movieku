...
class SectionsPagerAdapter(private val mContext: Context, fm: FragmentManager, private val favorite: Boolean) : FragmentPagerAdapter(fm, BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT) {
    companion object {
        @StringRes
    ...
    override fun getItem(position: Int): Fragment =
        when (position) {
            0 -> MovieFragment(favorite)
            1 -> TVShowFragment(favorite)
            else -> Fragment()
        }
